#!/usr/bin/env python3
"""
Compare two database schemas to find missing tables and columns.
"""

import re
from typing import Dict, List

def parse_tables(sql_content: str) -> Dict[str, List[str]]:
    """Parse SQL schema and extract table definitions with columns."""
    tables = {}
    ...
    for match in re.finditer(create_table_pattern, sql_content, re.DOTALL | re.IGNORECASE):
        ...
        for line in lines:
            ...
            # Track parentheses depth for constraints
            ...
            # Match column definitions (column name at start)
            col_match = re.match(r'^(\w+)\s+', line)
            if col_match and paren_depth == 0:
                # Extract full column definition up to comma or end
...
def extract_column_name(col_def: str) -> str:
    """Extract column name from column definition."""
    match = re.match(r'^(\w+)\s+', col_def)
    return match.group(1) if match else ""

def find_column_definition(column_defs: List[str], col_name: str) -> str:
    """Find full column definition by column name."""
    for col_def in column_defs:
        if col_def.startswith(col_name + ' '):
            return col_def
    return col_name

# Read the schema content from the attached files
# File 1 content (from Untitled-1)
file1_content = """-- DROP SCHEMA public;
...
# File 2 content would be too long, let me read it from the actual comparison
# I'll create a more efficient approach by directly comparing the key differences

if __name__ == "__main__":
    print("Analyzing schemas...")
    print("This will compare the two schema files to find differences.")
    print("Please run this with both schema files available.")
