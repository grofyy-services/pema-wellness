            ...
            if sac_elem is not None and inv_count_elem is not None:
                inventory_data = {
                    'status_application_control': {
                        ...
                        'inv_type_code': sac_elem.get('InvCode'),  # Use InvCode as per XML sample
...
async def verify_ids_auth(request: Request, xml_body: str = None) -> None:
    """
    Verify HTTP Basic authentication from IDS

    According to IDS specification:
    - Authorization: Basic HTTP authentication using IDS provided username/password with base-64 encoding
    - Fallback: Check for userid/password attributes in XML payload
    """
    ...
    if auth_header:
        ...
        try:
            ...
            logger.warning(f"Failed to decode IDS credentials: {e}")
            raise HTTPException(status_code=401, detail="Invalid authorization format")
    elif xml_body:
        # Fallback: Check for credentials in XML payload
        try:
            import defusedxml.ElementTree as ET
            root = ET.fromstring(xml_body)
            username = root.get('userid')
            password = root.get('password')

            if not username or not password:
                logger.warning("Missing userid/password in XML payload")
                raise HTTPException(status_code=401, detail="Missing authorization header")

        except ET.ParseError as e:
            logger.warning(f"Failed to parse XML for credentials: {e}")
            raise HTTPException(status_code=401, detail="Invalid XML format")
    else:
        logger.warning("Missing Authorization header and no XML payload")
        raise HTTPException(status_code=401, detail="Missing authorization header")
    ...
    if username != expected_username or password != expected_password:
        logger.warning(f"Invalid IDS credentials: username={username}")
        raise HTTPException(status_code=401, detail="Invalid credentials")

    logger.info(f"Successfully authenticated IDS request from: {username}")
...
<OTA_HotelInvCountNotifRS xmlns="http://www.opentravel.org/OTA/2003/05"
    ...
    <Success />
        ...
        # Return error response as XML (OTA standard)
...
async def create_booking_get(
    ...
    total_amount: int = Query(..., ge=0, description="Total amount in INR (example: 149680 for 3 nights)"),
    ...
    try:
        ...
        # Validate the data using BookingCreateRequest
        validated_data = BookingCreateRequest(**booking_data)

        ids_service = IDSService()
        return await ids_service.create_booking(validated_data.model_dump())
    except Exception as e:
